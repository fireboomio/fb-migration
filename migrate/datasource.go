	...
	for _, source := range fds {
		...
		case 2:
			...
			ds.CustomRest = &wgpb.CustomRest{
				OasFilepath: filepath.Join("oas", gjson.Get(sourceStr, "config.filePath").String()),
				BaseUrl:     gjson.Get(sourceStr, "config.baseURL").String(),
				Headers:     getHeaders(sourceStr),
			}
			_ = utils.CopyFile(filepath.Join(consts.UploadPath, ds.CustomRest.OasFilepath),
				filepath.Join(consts.UploadCloudPath, ds.CustomRest.OasFilepath))
			break
		case 3:
